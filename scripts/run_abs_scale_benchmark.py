...
CASES = [
    "eff01_midwidth_bitop_chain",
    "eff02_midwidth_arith_chain",
    "eff03_shift_cmp_mix",
    "eff04_deep_dependency_chain",
    "eff05_highwidth_mixed",
    "eff06_complex_combo",
]
...
def main() -> None:
    ...
    ap.add_argument("--out-csv", default="out/bench_eff.csv")
    ap.add_argument("--out-md", default="out/bench_eff.md")
    args = ap.parse_args()
    ...
    for case in cases:
        ...
        print(
            f"  width={rows[-1]['max_signal_width']} nodes={node_count} "
            f"avg={rows[-1]['avg_abs_time_s']}s"
        )
    ...
    print(f"[eff] wrote: {out_csv}")
    print(f"[eff] wrote: {out_md}")
